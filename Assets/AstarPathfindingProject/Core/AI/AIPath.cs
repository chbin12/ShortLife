...
public class AIPath : MonoBehaviour {
	...
	protected virtual void OnEnable () {
		...
		seeker.pathCallback += OnPathComplete;// modify by chenbin
	...
	public virtual void Update () {
		...
		} else if (rigid != null) {		//del by chenbin
			rigid.AddForce (dir);		//del by chenbin
		} else {
...
//#if ASTARDEBUG
		...
		if (Time.fixedDeltaTime	> 0) {		// modify by chenbin
			sp = Mathf.Clamp (sp,0,targetDist/(Time.fixedDeltaTime*2));		// modify by chenbin
		}
		return forward*sp;
	}
	...
	protected virtual void RotateTowards (Vector3 dir) {
		...
		rot = Quaternion.Slerp (rot,toTarget,turningSpeed*Time.fixedDeltaTime);// modify by chenbin
