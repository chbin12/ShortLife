...
public class CreatUnity3dType
{
	public delegate void CreateDelegate (string file,UnityEngine.Object obj);
	
	[@MenuItem("Coolape/Asset Maker/Build AssetBundles Selection Files 4 Upgrade", false, 0)]
	//这里不知道为什么用"@"，添加菜单
    static void makeAssetBundles ()//该函数表示通过上面的点击响应的函数
	{
...
//			path = path.Replace ("Assets/", "");
	...
	[@MenuItem("Coolape/Asset Maker/Build AssetBundles Selection Files 4 Upgrade(Uncompressed)", false, 0)]
	//这里不知道为什么用"@"，添加菜单
    static void makeAssetBundlesUncompressed ()//该函数表示通过上面的点击响应的函数
	{
		foreach (UnityEngine.Object o in Selection.objects) {
			Debug.Log (o.name);
			string path = AssetDatabase.GetAssetPath (o);//Selection表示你鼠标选择激活的对象
...
//			path = path.Replace ("Assets/", "");
	...
	[@MenuItem("Coolape/Asset Maker/Build AssetBundles From Directory of Files 4 Upgrade", false, 0)]
	//这里不知道为什么用"@"，添加菜单
    static void makeAssetBundlesSelections ()//该函数表示通过上面的点击响应的函数
	{
...
//	[@MenuItem("Coolape/Asset Maker/Make NGUI Assets")]
//	static void makeNguiAsset ()
//	{
//		string path = AssetDatabase.GetAssetPath (Selection.activeObject);//Selection表示你鼠标选择激活的对象
//		Debug.Log ("Selected Folder: " + path);
//		if(string.IsNullOrEmpty(path) || !Directory.Exists(path)) {
...
//		}
//		path = Application.dataPath + "/"  + path.Replace ("Assets/", "");
//		createUnity3dFiles(path, createNguiAssets, true);
//	}
...
//		string bundlePath  = Application.dataPath +  "/" + file + "/Android/" +obj.name +  ".unity3d";
...
#elif UNITY_IPHONE
//		string bundlePath  = Application.dataPath +  "/" + file + "/IOS/" +obj.name +  ".unity3d";
		bundlePath  = file + "/IOS/" +obj.name +  ".unity3d";
		Directory.CreateDirectory(Path.GetDirectoryName(bundlePath));
        BuildPipeline.BuildAssetBundle(obj, null, bundlePath, opt, BuildTarget.iPhone);
#endif
		FileInfo fileInfo = new FileInfo (bundlePath);
		long size = (fileInfo.Length / 1024);
		if (size >= 900) {
			Debug.LogWarning (" size== " + size + "KB");
		} else {
	...
	public class ExportAssetBundles
	{
		...
		static void ExportResource ()
		{
			...
			if (path.Length != 0) {
				...
				BuildPipeline.BuildAssetBundle (Selection.activeObject, selection, path, 
				                                ...
				                                BuildTarget.iPhone);
				#elif UNITY_ANDROID || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX
				BuildPipeline.BuildAssetBundle (Selection.activeObject, selection, path, 
				                                BuildAssetBundleOptions.CollectDependencies | BuildAssetBundleOptions.CompleteAssets, 
				                                BuildTarget.Android);
				#endif
		...
		static void ExportResourceNoTrackUncompress ()
		{
			...
			if (path.Length != 0) {
				// Build the resource file from the active selection.
				#if UNITY_IPHONE
				BuildPipeline.BuildAssetBundle (Selection.activeObject, Selection.objects, path, BuildAssetBundleOptions.UncompressedAssetBundle, BuildTarget.iPhone);
				#elif UNITY_ANDROID || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX
