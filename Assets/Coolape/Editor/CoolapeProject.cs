...
public class CoolapeProject : EditorWindow
{
	...
	void OnGUI()
	{
		...
		GUILayout.Space(5);
		using (new FoldableBlock(ref state1, "Project init  " + (state1 ? "(Click to hide)" : "(Click to show)"))) {
			if (state1) {
...
//		{
		using (new FoldableBlock(ref state2, "Refresh AssetBundles  " + (state2 ? "(Click to hide)" : "(Click to show)"))) {
			if (state2) {
				using (new HighlightBox()) {
...
//		NGUIEditorTools.BeginContents ();
//		{
		using (new FoldableBlock(ref state3, "Build  " + (state3 ? "(Click to hide)" : "(Click to show)"))) {
			if (state3) {
				...
				using (new HighlightBox()) {
					...
					GUI.color = Color.red;
					GUILayout.Toggle(SCfg.self.isUseEncodedLua, "Use encoded Lua (setting in the Cfg object) ");
