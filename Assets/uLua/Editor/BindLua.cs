...
//        _GT(typeof(CharacterController)),
		...
		_GT(typeof(SystemInfo)),	// add by chenbin
		_GT(typeof(PlayerPrefs)), // add by chenbin
		_GT(typeof(YieldInstruction)),	// add by chenbin
		_GT(typeof(Coroutine)),	// add by chenbin
		_GT(typeof(LineRenderer)),	// add by chenbin
		_GT(typeof(DeviceOrientation)),	// add by chenbin
	...
	public static List<BindType> addBindType() {
		string path = Application.dataPath + "/uLua/Editor/WrapClassCfg.json";
		ArrayList list =  Toolkit.JSON.DecodeList(File.ReadAllText(path));
		List<BindType> ret = new List<BindType>();
		int count = list.Count;
		string className = "";
		for(int i =0; i < count; i++) {
			className = list[i].ToString();
//			Type tp = Type.GetType(className);
			/* 通过类名取得type，因为是在editor环境下，
			直接用Type.getType(name)是不能取得非editor环境下的类型的，
			只能通过运行类里的方法取得相应该的类型
			*/
			Type tp = Util.getTypeByName (className);
			ret.Add(_GT(tp));
		}
		return ret;
	}
	
	public static void setMemberFilter() {
		ToLuaExport.memberFilter.Clear();
		string path = Application.dataPath + "/uLua/Editor/MemberFilter.json";
		ArrayList list =  Toolkit.JSON.DecodeList(File.ReadAllText(path));
		int count = list.Count;
		for(int i = 0; i < count; i++) {
			ToLuaExport.memberFilter.Add(list[i].ToString());
		}
	}
	#endregion
    ...
    public static void Binding()
    {
		...
		btList.AddRange(binds);
		btList.AddRange(addBindType());
		BindType[] list = btList.ToArray();
		#endregion
