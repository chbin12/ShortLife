...
public class UICameraWrap
{
	public static void Register(IntPtr L)
	{
		LuaMethod[] regs = new LuaMethod[]
		{
			...
			new LuaMethod("ProcessRelease", ProcessRelease),
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("dragThreshold", get_dragThreshold, null),
		};
	...
	static int get_dragThreshold(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name dragThreshold");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index dragThreshold on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.dragThreshold);
		return 1;
	}
	...
	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int ProcessRelease(IntPtr L)
	{
		...
		else if (count == 3)
		{
			...
			obj.ProcessRelease(arg0,arg1);
			return 0;
		}
		else
		{
			LuaDLL.luaL_error(L, "invalid arguments to method: UICamera.ProcessRelease");
		}
