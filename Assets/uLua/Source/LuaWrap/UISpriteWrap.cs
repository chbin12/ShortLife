...
public class UISpriteWrap
{
	public static void Register(IntPtr L)
	{
		LuaMethod[] regs = new LuaMethod[]
		{
			...
			new LuaMethod("unSetGray", unSetGray),
		...
		{
			new LuaField("atlasName", get_atlasName, set_atlasName),
			new LuaField("material", get_material, null),
			new LuaField("grayMaterial", get_grayMaterial, set_grayMaterial),
	...
	static int get_atlasName(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			{
				LuaDLL.luaL_error(L, "unknown member name atlasName");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index atlasName on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.atlasName);
		return 1;
	}
	...
	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int get_grayMaterial(IntPtr L)
	{
		object o = LuaScriptMgr.GetLuaObject(L, 1);
		UISprite obj = (UISprite)o;
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name grayMaterial");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index grayMaterial on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.grayMaterial);
		return 1;
	}
	...
	static int set_atlasName(IntPtr L)
	{
		...
		obj.atlasName = LuaScriptMgr.GetString(L, 3);
		return 0;
	}

	[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	static int set_grayMaterial(IntPtr L)
	{
		object o = LuaScriptMgr.GetLuaObject(L, 1);
		UISprite obj = (UISprite)o;
		...
		obj.grayMaterial = (Material)LuaScriptMgr.GetUnityObject(L, 3, typeof(Material));
		return 0;
	}
