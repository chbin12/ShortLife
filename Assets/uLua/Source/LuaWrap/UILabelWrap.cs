...
public class UILabelWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("fontName", get_fontName, set_fontName),
	...
	static int get_fontName(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name fontName");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index fontName on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.fontName);
		return 1;
	}
	...
	static int set_fontName(IntPtr L)
	{
		...
		obj.fontName = LuaScriptMgr.GetString(L, 3);
		return 0;
	}
