	...
	void OnGUI() {
		if(isShowGUI) {
			...
			if(label != null && UICamera.currentCamera != null)
				pos = UICamera.currentCamera.WorldToScreenPoint(transform.position);
			rect = new Rect( pos.x, Screen.height - pos.y-collider.size.y/2, collider.size.x, collider.size.y);
			GUI.SetNextControlName("__MyTextField");
			GUI.color = ColorEx.getColor(0,0,0,0);
