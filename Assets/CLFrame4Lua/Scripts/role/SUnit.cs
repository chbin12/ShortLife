...
public abstract class  SUnit : CLBehaviour4Lua
{ 
	public static float SCANRange = 30;		//视野
	public int instanceID = 0;			//id
	[HideInInspector]
	public int type;							//类型				
//	[HideInInspector]
	public int id;								//id
	...
	public int lev {
		get {
			return Toolkit.NumEx.bio2Int(_lev);
		}
		set {
			_lev = Toolkit.NumEx.int2Bio(value);
		}
	}

	public SUnit mTarget;				//目标
	public SUnit mAttacker;			//攻击我的对象
	[HideInInspector]
	public SBSliderBar _sliderLifeBar;		//血条
	...
	public Transform hudAnchor;				//HUD的锚点
	public CLRoleState state = CLRoleState.idel;		//状态
	[HideInInspector]
	public bool isOffense;							//是否进攻方
	public bool isDefense {
		get {
			return !isOffense;
		}
	}
	public bool isCopyBody = false;			//是否分身
...
//			return;
	...
	public float initRandomFactor() {
		RandomFactor = Toolkit.NumEx.NextInt (0, 1001)/1000.0f;
		return RandomFactor;
	}
...
//		setMatToon ();
	...
	public abstract SUnit doSearchTarget ();

	public abstract void onBeTarget (SUnit attacker);

	public abstract void onRelaseTarget (SUnit attacker);

	public abstract void doAttack ();

	public abstract void onHurtHP (int hurt, object skillAttr);
	public abstract bool onHurt (int hurt, object skillAttr, SUnit attacker);
	public abstract void onHurtFinish (object skillAttr, SUnit attacker);
