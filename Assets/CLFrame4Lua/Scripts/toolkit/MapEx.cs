...
namespace Toolkit
{
	public class MapEx
	{
		...
		static byte[] _getBytes (LuaInterface.LuaTable map, object key)
		{
			try {
        ...
        public static int getBytes2Int(Hashtable map, object key) {
            if(map == null || key == null) return 0;
            if(SCfg.self.useBio4Battle){
                return NumEx.bio2Int(getBytes(map, key));
            } else {
                return getInt(map, key);
            }
        }
        public static void setInt2Bytes(Hashtable map, object key, int val) {
            if(map == null || key == null) return;
            if(SCfg.self.useBio4Battle){
                map[key] = NumEx.int2Bio(val);
            } else {
                map[key] = val;
            }
        }
		...
		{
			try {
				if(map == null) return new Hashtable ();
				if (!map.Contains (key))
