...
public class CLModelMgr : MonoBehaviour
{
	...
	[SerializeField]
	public List<CLAvatar>
		dataAvatar = new List<CLAvatar> ();
...
public class CLAvatar
{
	...
	public string avatar4Editor;
	[System.NonSerialized]
	static Hashtable
		avatarPathCache = new Hashtable ();
	
	public string avatar {
		get {
			string avatarPath = MapEx.getString (avatarPathCache, avatar4Editor);
			if (string.IsNullOrEmpty (avatarPath) && !string.IsNullOrEmpty (avatarPath)) {
				avatarPath = PStr.b ().a (Path.GetDirectoryName (avatar4Editor)).a ("/").a (PathCfg.self.platform).a ("/").a (Path.GetFileNameWithoutExtension (avatar4Editor)).a (".unity3d").e ();
				avatarPath = avatarPath.Replace ("/upgradeResMedium/", "/upgradeRes/");
				avatarPathCache [avatar4Editor] = avatarPath;
			}
			return avatarPath;
		}
	}
