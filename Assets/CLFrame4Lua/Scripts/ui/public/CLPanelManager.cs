...
//	public UnityEngine.Object luaText;
//	LuaState _uiLua;
//	
//	public void initLuaBuilder()
//	{
//		string path = PathCfg.persistentDataPath + "/" + PathCfg.self.panelLuaBuilder;
//#if UNITY_EDITOR
//		if(Application.isPlaying) {
//			_uiLua = Main.self.lua;//new LuaState ();
//		} else {
//			_uiLua = new LuaState();	
//		}
//#else
//		_uiLua = Main.self.lua;//new LuaState ();
//#endif
//		Utl.doLua(_uiLua, path);
//	}
//	
//	public LuaState uiLua {
//		get {
//			if (_uiLua == null) {
//				initLuaBuilder();
//			}
...
//        yield return null;
		...
		if (panelRetainLayer.Count > 0) {
			if (!isRetainCurr) {
				self.depth -= 100;
				self.depth = self.depth < 100 ? 100 : self.depth;
...
//		}
	...
	public static void getPanelAsy(string pName, object callback, object paras)
	{
		...
		if (p != null) {
			if (callback != null) {
				if (callback.GetType() == typeof(LuaFunction)) {
					((LuaFunction)callback).Call(p, paras);
				} else if (callback.GetType() == typeof(Callback)) {
	...
	static IEnumerator loadPanel(string pName, object callback, object paras)
	{
        if(SCfg.self.isEditMode) {
            string path = PStr.begin().a("file://").a(PathCfg.persistentDataPath).a("/")
            ...
            string path = PStr.begin().a(PathCfg.self.panelDataPath).a(PathCfg.self.platform).a("/").a(pName).a(".unity3d").end();
	...
	public static void finishGetPanel(string pName, AssetBundle ab, object callback, object paras) {
		if (ab != null) {
			...
			if(p.isNeedResetAtlase) {
				resetAtlasAndFont(p.transform, false);
			}
			panelBuff [pName] = p;
			if(p.name != CLMain.self.startPanel) {
...
//			pop.bitmapFont = font;
		...
		for (int i =0; i < tr.childCount; i++) {
			resetAtlasAndFont(tr.GetChild(i), isClean);
		}
	}
