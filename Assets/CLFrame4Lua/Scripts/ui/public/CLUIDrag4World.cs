...
5.特别注意groundMask，这个可以处理能够看到的地表的大小，因此需要一块在表的boxCollider
*/
...
public class CLUIDrag4World : MonoBehaviour
{
	...
	int _dragProcType = -1;//0:拖动 1:旋转 2:缩放
	int dragProcType {
		get {
	...
	void Move (Vector3 worldDelta, bool isDoLimitView = false)
	{
		...
		bool inGround = true;
		if(isDoLimitView) {
			inGround = limitDisplayView();
		} else {
			inGround = isInviewBounds ();
		}
		if (!inGround) {
			if(!isDoLimitView) {
			...
			if (onDragMoveDelegate != null) {
				if (onDragMoveDelegate is LuaInterface.LuaFunction) {
					((LuaInterface.LuaFunction)onDragMoveDelegate).Call ();
				} else if (onDragMoveDelegate is Callback) {
					((Callback)onDragMoveDelegate) ();
				}
			}
		}
	}
	...
	bool isInviewBounds ()
	{
		if (!getViewPoint2World (new Vector3 (0, Screen.height / 2, 0), ref viewLeft)) {
			return false;
		}
		if (!getViewPoint2World (new Vector3 (Screen.width, Screen.height / 2, 0), ref viewRight)) {
			return false;
		}
		if (!getViewPoint2World (new Vector3 (Screen.width / 2, Screen.height, 0), ref viewTop)) {
			return false;
		}
		if (!getViewPoint2World (new Vector3 (Screen.width / 2, 0, 0), ref viewBottom)) {
			return false;
		}
		...
		if (mMomentum.magnitude > 0.0001f) {
			if (!mPressed) {
				...
				if (mMomentum.magnitude < 0.0001f) {
					...
					if (onEndDragMoveDelegate != null) {
						if (onEndDragMoveDelegate is LuaInterface.LuaFunction) {
							((LuaInterface.LuaFunction)onEndDragMoveDelegate).Call ();
						} else if (onEndDragMoveDelegate is Callback) {
							((Callback)onEndDragMoveDelegate) ();
						}
					}
				}
			} else {
